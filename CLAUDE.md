...
- **Self-describing boards.** Ship commands that let an agent learn a board's schema at runtime *before* acting: `items list-columns`, `statuses list`, `subitems list-columns`, `subitems list-statuses`, `columns list`. Any new resource should provide an equivalent discovery command. The `columns` group also *manages* board column structure — `columns create` (with status/dropdown label seeding), `columns update` (rename + `--add-label`), `columns delete` — so a board can be fully provisioned from the CLI.
...
- **Deterministic and non-interactive.** Anything scriptable must run without a TTY prompt: `--all` for pagination, and a flag to skip delete confirmations. (Note: this skip flag is mostly `--force/-f` — `items delete`, `subitems delete`, and `columns delete` all use it; only the older `groups delete` still uses `--confirm/-y`. Converge new destructive commands on `--force/-f`; consistency is itself a discoverability feature.)
